...
from model import Patient, Doctor, Expertise, Office, Position, Information, Medicine, OrderForMedicine, Pharmacy, Registration, Room, Section, Timeslot, Appointment
...
class PatientRepository(Base):
    def get_all_patients(self) -> List[Patient]:
        ...
        return [Patient.from_dict(row) for row in results] if results else []
    ...
    def get_patient_by_id(self, patient_id: int) -> Optional[Patient]:
        ...
        return Patient.from_dict(result[0]) if result else None
